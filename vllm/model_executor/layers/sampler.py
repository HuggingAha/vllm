...
class Sampler(nn.Module):
    ...
    def _init_sampling_tensors(
        ...
        (sampling_tensors, do_penalties, do_top_p_top_k, do_min_p, do_top_n_sigma) = (
        ...
        self._do_top_n_sigma = do_top_n_sigma
    ...
    def forward(
        ...
        * Encapsulate arguments required for deferred Pythonization
        ...
        do_top_n_sigma = self._do_top_n_sigma
        ...
        if do_top_n_sigma:
            logits = _apply_top_nsigma(logits, sampling_tensors.top_n_sigmas)
...
def _apply_top_nsigma(
    ...
    """Apply top-n-sigma sampling to the logits.

    Args:
        logits: Token logits of shape [batch_size, vocab_size]
        n_sigma: Number of standard deviations to use as threshold, shape [batch_size]

    Returns:
        Modified logits with same shape as input
    """
    # Calculate mean and std along vocab dimension
    # mean = logits.mean(dim=-1, keepdim=True)  # [batch_size, 1]
    std = logits.std(dim=-1, keepdim=True)  # [batch_size, 1]

    # Calculate threshold for each sequence in batch
    # threshold = mean + n_sigma * std
    threshold = logits.max(dim=-1, keepdim=True).values - n_sigma.unsqueeze(dim=1) * std

    # Mask logits below threshold with -inf
    logits = logits.masked_fill_(logits < threshold, -float("inf"))
...
def _beam_search_sample(
    ...
    for seq_group in selected_seq_groups:
        ...
        if is_prompt:
            ...
            # Generation phase.
            cumulative_logprobs: List[float] = [seq_group.seq_data[seq_id].cumulative_logprob for seq_id in seq_ids]
    ...
    * Defer Pythonization & preserve GPU-side
    ...
    if include_gpu_probs_tensor:
        sampled_token_ids_tensor = torch.full(
            (logprobs.shape[0], 1), VLLM_INVALID_TOKEN_ID, dtype=torch.long, device=logprobs.device
        )
    else:
        ...
        if sampling_type == SamplingType.GREEDY:
            ...
            if flashinfer_top_k_top_p_sampling is not None:
                ...
                multinomial_samples[sampling_type] = _multinomial(
                    probs[long_sample_indices], max_n_in_batch, seq_groups=seq_groups_arg
                )
...
def get_logprobs(
    ...
    for seq_group, sample_result in zip(sampling_metadata.seq_groups, sample_results):
        (prompt_logprobs, top_logprob_idx, selected_logprobs_idx) = _get_prompt_logprob_if_needed(
            seq_group, selected_logprobs, ranks, top_token_ids, top_logprobs, selected_logprobs_idx, top_logprob_idx
        )
        ...
        for idx, token_id in enumerate(next_prompt_tokens):
            ...
            if num_logprobs > 0:
                ...
                prompt_logprobs_dict.update({
                    top_id: (top_prob, rank) for top_id, top_prob, rank in zip(top_ids, top_probs, top_ranks)
                })
            prompt_logprobs.append({
                token_id: Logprob(*logprob_and_rank) for token_id, logprob_and_rank in prompt_logprobs_dict.items()
            })
        ...
        if num_logprobs is None:
            ...
            for idx, (next_token_id, parent_id) in enumerate(zip(next_token_ids, parent_seq_ids)):
                ...
                sampled_logprobs.append({
                    token_id: Logprob(*logprob_and_rank) for token_id, logprob_and_rank in sampled_logprobs_dict.items()
                })
...
def _build_sampler_output(
    ...
    if skip_sampler_cpu_output:
        ...
        for seq_group, sample_result, group_prompt_logprobs, group_sample_logprobs in zip(
            sampling_metadata.seq_groups, maybe_deferred_sample_results, prompt_logprobs, sample_logprobs
        ):
            seq_ids = seq_group.seq_ids
